...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescription('Alice_BOTが生成したWebHookをすべて削除します。'),
  ...
  async execute(interaction) {
    try {
      ...
      if (aliceWebhooks.size === 0) {
        await interaction.reply('Alice_BOTが生成したWebHookは見つかりませんでした。');
        return;
      }
      ...
      await interaction.reply('Alice_BOTが生成したWebHookをすべて削除しました。');
    } catch (error) {
      console.error(error);
